...
import {option} from "../config/sweetAlert";
...
export function errorHandler(err) {
  if (!navigator.onLine) {
    history.push(`/${variables.PAGE.ERROR.VALUE}`);
...
//   return momentJalaali(timestamp * (milisecond ? 1 : 1000)).fromNow();
// }
...
* @Param {String} confirmText - A confirm button text
* */
export function successAlert(title = '', text = '', confirmText = '', timer) {
  const newAlert = alert();
...
export function httpErrorAlert(title = '', text = '', cancelText = '', error = null, canReport = false) {
  // const token = localStorage.getItem(variables.ACCESS_TOKEN);
  // const loggedInUser = store.getState().user.sejamUser;
  const newAlert = alert();
  newAlert.fire({
    customClass: {
      container: 'sweet-alert__container',
      ...
      content: 'sweet-alert__content',
      icon: 'sweet-alert__icon',
      confirmButton: 'uk-button sweet-alert-report-button sweet-alert-report-button-default margin__horizontal__5',
      cancelButton: 'uk-button uk-button-primary margin__horizontal__5 flex-1',
    },
    title: title ? title : 'خطا!',
    text: text ? text : "متاسفانه مشکلی پیش آمده است. لطفا دوباره امتحان کنید.",
    icon: 'error',
    showCancelButton: true,
    // showConfirmButton: loggedInUser && (Object.keys(loggedInUser).length !== 0) && loggedInUser.firstName && token,
    showConfirmButton: canReport,
    cancelButtonText: cancelText ? cancelText : 'باشه',
    confirmButtonText: "گزارش خطا"
  }).then((result) => {
    // if (result.value && canReport && error) {
    //   ticket(error);
    // }
  });
}
// export function ticket(error = null) {
//   // const token = localStorage.getItem(variables.ACCESS_TOKEN);
//   const loggedInUser = store.getState().user.sejamUser;
//   const newAlert = alert();
//   newAlert.fire({
//     customClass: {
//       container: 'sweet-alert__container',
//       popup: 'sweet-alert__error__report__popup',
//       title: 'sweet-alert__title',
//       content: 'sweet-alert__content',
//       icon: 'sweet-alert__icon',
//       confirmButton: 'uk-button uk-button-primary margin__horizontal__5 flex-1',
//       cancelButton: 'uk-button sweet-alert-report-button sweet-alert-report-button-default margin__horizontal__5',
//     },
//     title: 'گزارش خطا',
//     content: "input",
//     input: 'textarea',
//     inputPlaceholder: 'متن گزارش خود را بنویسید.',
//     inputAttributes: {
//       'aria-label': 'متن گزارش خود را بنویسید.'
//     },
//     showCancelButton: true,
...
//           resolve()
//         } else {
//           resolve('لطفا متن گزارش را پر نمایید')
//         }
//       })
//     },
//     preConfirm: (text) => {
//       return userApi.createTicket({
//         title: `گزارش خطا برای کاربر ${loggedInUser.firstName + " " + loggedInUser.lastName}`,
//         description: text,
//         nationalCode: loggedInUser.nationalCode,
//         sejamTrackingCode: loggedInUser.sejamTrackingCode,
//         data: {
//           nationalCode: loggedInUser.nationalCode,
//           sejamTrackingCode: loggedInUser.sejamTrackingCode,
//           token: localStorage.getItem(variables.ACCESS_TOKEN),
//           ...(loggedInUser.sejamTrackingCode ? {sejamTrackingCode: loggedInUser.sejamTrackingCode}: {}),
//           ...(error ? {
//             mobile: loggedInUser.mobile,
//             requestUrl: error.config.url,
//             requestData: error.config.data,
//             token: error.config.headers["Authorization"].split("Bearer ")[1],
...
//     },
//       // allowOutsideClick: () =>
//       allowOutsideClick: () => !newAlert.isLoading()
//     }).then((result) => {
//       if (result.value) {
//         newAlert.fire({
//           customClass: {
...
//             icon: 'sweet-alert__icon',
//             confirmButton: 'uk-button uk-button-primary margin__horizontal__5 flex-1',
//             cancelButton: 'uk-button uk-button-default-important uk-button-important margin__horizontal__5',
//           },
//           icon: 'success',
...
export function parseQuery(query) {
  const queryData = query.replace("?", "").split("&");
...
/*
* Get config value from local storage
* @Param {String} key - A key of config
* @Param {Any} defaultValue - A default config value if config and config key is not exist
* @Return {Any}
* */
// export function getConfigValue(key, defaultValue = '') {
//   if (!key) return null;
//
//   const configs = JSON.parse(localStorage.getItem(variables.CONFIG));
//   return configs && configs[key] ? configs[key] : defaultValue;
// }
...
function isValidAttrs(attr) {
  ...
  const attrs = ['className', 'id', 'onclick'];
...
export const toast = {
  ...
  addTranslateY(toast) {
    ...
    let toasts = [...toast.parentElement.getElementsByClassName(this.preClass)];
  ...
  /*appendChildren(toast, icon, trashIcon, content,  text,  controller,  confirmButton,  rejectButton,  close,  closeIcon) {
    const tree = {
      element: toast,
      children: [
        {
  ...
  test(tree) {
    // const parent = tree.element;
    const child = !isEmptyArray(tree.children) ? this.test(tree.children[0]) : tree.element;
  },*/

  alert(body = '', target = 'body', option = {}) {
    return new Promise( (resolve, reject) => {
      try {
        const targetElement = document.querySelectorAll(target)[0];
        ...
        const toastElement = this.createToast({...TOAST_ALERT_OPTION, ...option}, resolve, reject);
  ...
  confirm(body = ' ', target = 'body', option = {})   {
    return new Promise((resolve, reject) => {
      try {
        const targetElement = document.querySelectorAll(target)[0];
        if (isEmpty(targetElement)) return reject('Toast target not found');
